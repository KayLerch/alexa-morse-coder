...
public class MorseUserTest {
    ...
    public void getSetIsAskedForNewExercise() throws Exception {
        user.setIsAskedForNewExercise(false);
        assertFalse(user.getIsAskedForNewExercise());
        user.setIsAskedForNewExercise(true);
        assertTrue(user.getIsAskedForNewExercise());
    }

    @Test
    public void withIsAskedForNewExercise() throws Exception {
        final MorseUser user2 = user.withIsAskedForNewExercise(false);
        assertEquals(user, user2);
        assertFalse(user2.getIsAskedForNewExercise());
        user.withIsAskedForNewExercise(true);
        assertTrue(user.getIsAskedForNewExercise());
    }

    @Test
    public void getSetIsAskedForAnotherEncode() throws Exception {
        user.setIsAskedForAnotherEncode(false);
        assertFalse(user.getIsAskedForAnotherEncode());
        user.setIsAskedForAnotherEncode(true);
        assertTrue(user.getIsAskedForAnotherEncode());
    }

    @Test
    public void withIsAskedForAnotherEncode() throws Exception {
        final MorseUser user2 = user.withIsAskedForAnotherEncode(false);
        assertEquals(user, user2);
        assertFalse(user2.getIsAskedForAnotherEncode());
        user.withIsAskedForAnotherEncode(true);
        assertTrue(user.getIsAskedForAnotherEncode());
    }

    @Test
    public void getSetIsAskedForAnotherTry() throws Exception {
        user.setIsAskedForAnotherTry(false);
        assertFalse(user.getIsAskedForAnotherTry());
        user.setIsAskedForAnotherTry(true);
        assertTrue(user.getIsAskedForAnotherTry());
    }

    @Test
    public void withIsAskedForAnotherTry() throws Exception {
        final MorseUser user2 = user.withIsAskedForAnotherTry(false);
        assertEquals(user, user2);
        assertFalse(user2.getIsAskedForAnotherTry());
        user.withIsAskedForAnotherTry(true);
        assertTrue(user.getIsAskedForAnotherTry());
    }
    ...
    public void withNothingAsked() throws Exception {
        user.setIsAskedForAnotherTry(true);
        final MorseUser user2 = user.withNothingAsked();
        assertEquals(user, user2);
        assertFalse(user.getIsAskedForAnotherTry());

        user.setIsAskedForNewExercise(true);
        final MorseUser user3 = user.withNothingAsked();
        assertEquals(user, user2);
        assertFalse(user.getIsAskedForNewExercise());

        user.setIsAskedForNewExercise(true);
        final MorseUser user4 = user.withNothingAsked();
        assertEquals(user, user2);
        assertFalse(user.getIsAskedForNewExercise());
    }
