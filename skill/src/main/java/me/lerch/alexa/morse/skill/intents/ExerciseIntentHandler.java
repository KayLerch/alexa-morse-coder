...
import static me.lerch.alexa.morse.skill.utils.SkillConfig.ScoreDecreaseOnFarnsworth;
import static me.lerch.alexa.morse.skill.utils.SkillConfig.ScoreDecreaseOnRetry;
...
public class ExerciseIntentHandler extends AbstractIntentHandler {
        ...
        try {
            ...
            if (exercise.isPresent()) {
                ...
                if (hasExerciseCorrect(intent, exerciseForSure.getLiteral())) {
                    ...
                    final Integer score = user.getWpm() - (user.getFarnsworthEnabled() ? ScoreDecreaseOnFarnsworth : 0);
                ...
                // answer is not correct
                    ...
                    user.withDecreasedPersonalScoreBy(ScoreDecreaseOnRetry)
    ...
    private AlexaSpeechletResponse getWrongAnswerResponse() {
        return ask().withSsml(ResponsePhrases.getWasNotCorrect() + " <p>" + ResponsePhrases.getWantToTryItAgain()).build();
    }
    ...
    private boolean hasExerciseCorrect(final Intent intent, final String word) {
        ...
        return new DoubleMetaphone().isDoubleMetaphoneEqual(intentWord, word);
    }
}
